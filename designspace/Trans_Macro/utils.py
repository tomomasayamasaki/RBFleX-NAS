...
def get_config_tranmicro(CONFIG_PATH):
    # load config
    with open(os.path.join('./designspace/Trans_Macro/configs', CONFIG_PATH+'.yaml'), 'r') as f:
        config = utils.yaml_parser(yaml.unsafe_load(f))
    
    return config

def get_searchspace_tranmicro(config):
    with open(os.path.join('./designspace/Trans_Macro/', config.arch_config.train_archs_accs), 'r') as t:
